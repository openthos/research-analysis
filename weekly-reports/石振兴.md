...
- 张蔚帮着看了OS实验里的问题。1，现在的OS代码还不能同时运行在riscvemu和qemu里，因为在qemu里不再支持htif这种交互方式了。更深层的问题是需要了解riscemu究竟模拟的是哪种平台，能否和qemu里的某种硬件相对应，应该是要改riscemu的。2，从多个角度排查了中断切换问题，但还未找到问题的原因。
